...
type FigureProps = { src: string; width: number; height: number; project: string; status: string; caption: string; alt: string; className?: string; imageClassName?: string; imageStageClassName?: string; priority?: boolean };
function Figure({ src, width, height, project, status, caption, alt, className = "", imageClassName = "", imageStageClassName = "", priority = false }: FigureProps) {
  const image = <Image src={src} width={width} height={height} alt={alt} sizes="(max-width: 768px) 100vw, (max-width: 1280px) 50vw, 760px" priority={priority} className={`h-auto w-full ${imageClassName}`} />;
...
const evidencePair = `${evidenceCanvas} grid gap-8 md:grid-cols-2 md:items-start`;
const evidenceSingle = "w-full max-w-[760px]";
const evidenceCompactSingle = "w-full max-w-[660px]";
const evidenceSecondary = "w-full max-w-[640px]";
  ...
  return <main className="min-h-screen bg-[#F3F0EB] text-[#4A433D]">
    ...
    <article>
      ...
      <Section>
        <div>
          <div className="max-w-[760px]"><Heading number="01" title="수납보다 먼저 생활을 봅니다" /><Copy>
            ...
            <p>요리의 빈도, 보관해야 할 물건, 가족의 생활시간과 공간을 사용하는 방식이 다르면 같은 평면에서도 계획의 답은 달라집니다.</p>
        ...
        <aside className="mt-20 border-y border-[#675B56]/25 py-10 md:mt-28 md:py-14">
          ...
          <figure className="mx-auto mt-10 w-full max-w-[880px]">
            <div className="grid gap-6 md:hidden">
              <Image
                src="/knowledge/and-standards/storage-flow/suwon-entry-storage-flow-before-mobile.png"
                width={1024}
                height={832}
                sizes="100vw"
                alt="수원 살구골 현대7단지 현관 가까운 수납을 만들기 전 공간 관계"
                className="h-auto w-full"
              />
              <Image
                src="/knowledge/and-standards/storage-flow/suwon-entry-storage-flow-after-mobile.png"
                width={1024}
                height={832}
                sizes="100vw"
                alt="수원 살구골 현대7단지 현관 가까운 팬트리를 만든 뒤의 공간 관계"
                className="h-auto w-full"
              />
            </div>
            <div className="hidden md:block">
              <Image
                src="/knowledge/and-standards/storage-flow/suwon-entry-storage-flow-diagram.png"
                width={2048}
                height={832}
                sizes="(max-width: 768px) 100vw, 880px"
                alt="현관 인접 팬트리를 만들기 전후의 수원 살구골 현대7단지 공간 관계 개념도"
                className="h-auto w-full"
              />
            </div>
            <figcaption className="mt-4 text-[13px] leading-6 text-neutral-600 break-keep md:text-sm md:leading-7">
              거실 일부를 활용해 현관 가까운 수납과 거실의 중심면을 함께 만든 공간 관계.
            </figcaption>
          </figure>
          <div className="mt-8 grid max-w-[880px] gap-x-10 gap-y-7 md:grid-cols-2">{[
          ...
          ].map(([title, body]) => <div key={title} className="border-t border-[#675B56]/20 pt-5"><h3 className="text-[15px] font-medium leading-6 text-[#4A433D]">{title}</h3><p className="mt-2 text-sm leading-7 text-neutral-600 break-keep">{body}</p></div>)}</div>
      ...
      <Section><div>
        ...
        <div className="mt-14"><Copy><p>이 집은 요리를 자주 하는 가족의 생활을 기준으로 주방의 기능을 배치했습니다.</p><p>이 주방에서 중요한 것은 가구의 형태보다 <Strong>각 기능이 어떤 순서로 연결되는가</Strong>입니다.</p><p>이 원칙은 주방에만 적용되지 않습니다. 현관에는 귀가와 외출의 순서가 있고, 드레스룸에는 보관과 준비의 순서가 있습니다.</p><p><Strong>물건을 어디에 둘 것인가를 결정하기 전에, 그 물건을 언제 어떻게 사용하는지를 먼저 봐야 합니다.</Strong></p></Copy>
          ...
          <div className={evidenceCanvas}>
            <Figure src="/projects/apartment-b/12-kitchen-front.webp" width={1448} height={1086} project="화성 동탄역 모아미래도 84A" status="COMPLETED" caption="냉장·보관 영역과 중앙 아일랜드가 실제 사용 흐름 안에서 이어집니다." alt="냉장 수납벽과 중앙 아일랜드가 함께 보이는 동탄역 모아미래도 주방" className={evidenceCompactSingle} />
            <ProjectLink status="COMPLETED" href="/projects/apartment-b">동탄역 모아미래도에서 주방과 수납 계획 보기 →</ProjectLink>
          </div>
        </div>
      </div></Section>

      <Section><div className="max-w-[760px]"><Heading number="03" title="공간이 부족하면 기능의 위치부터 다시 봅니다" /><Copy><p>공간이 부족할 때 가장 먼저 떠올리는 방법은 각 요소를 조금씩 줄이는 것입니다.</p><p>수납장을 줄이고, 식탁을 작게 바꾸고, 통로를 좁히면서 필요한 기능을 기존 위치에 모두 남겨두려 합니다.</p><p>하지만 하나의 공간 안에서 여러 기능이 서로 경쟁하고 있다면, 크기를 줄이는 것만으로는 근본적인 문제가 해결되지 않을 수 있습니다.</p><p><Strong>이럴 때 AND는 각 기능을 얼마나 줄일 수 있는지를 보기보다, 기능 사이의 관계와 주변 동선까지 함께 놓고 위치를 다시 검토합니다.</Strong></p></Copy></div>
        ...
        <p className="mb-7 mt-12 text-xs font-medium tracking-[0.24em] text-neutral-500">RETHINKING POSITION</p><div className={evidencePair}>
          <Figure src="/projects/apartment-a/18-before-balcony.webp" width={1050} height={1400} project="수원 살구골 현대7단지 99" status="BEFORE" caption="활용도가 낮았던 기존 인접 영역" alt="기능을 옮기기 전 수원 살구골 현대7단지의 비어 있는 발코니" imageClassName="md:h-[680px] md:object-contain md:object-left" />
          <Figure src="/projects/apartment-a/07-utility-room.webp" width={1746} height={2400} project="수원 살구골 현대7단지 99" status="COMPLETED" caption="주방의 흐름을 유지하면서 인접 영역으로 기능의 위치를 조정했습니다." alt="냉장 기능과 유틸리티를 인접 영역으로 옮긴 수원 아파트 발코니" imageClassName="md:h-[680px] md:object-contain md:object-left" />
        </div>
        <div className={evidenceCanvas}><ProjectLink status="COMPLETED" href="/projects/apartment-a">수원 살구골 현대7단지에서 주방과 다이닝의 관계를 다시 정리한 방식 보기 →</ProjectLink></div></div>
      </Section>
      ...
      <aside className="mx-auto mb-32 max-w-[1060px] px-5 md:mb-48 md:px-16"><div className="border-y border-[#675B56]/30 py-12 md:py-16"><p className="text-[10px] font-medium tracking-[0.28em] text-neutral-500 md:text-xs">DESIGN NOTE</p><h2 className="mt-5 text-2xl font-light md:text-3xl">좋은 계획은 실제로 구현될 수 있어야 합니다</h2>
        <div className="mt-9 grid gap-10 lg:grid-cols-2 lg:gap-16"><Copy><p>생활에 맞는 좋은 아이디어도 실제 공간의 구조와 설비 조건 안에서 구현될 수 있어야 합니다.</p><p>마감 뒤에 가려지는 기존 조건 역시 공간의 높이와 마감, 가구와 조명의 관계를 결정하는 설계 조건입니다.</p><p><Strong>AND는 이런 조건을 마지막에 해결해야 할 문제로 남겨두기보다, 처음부터 공간을 구성하는 조건의 하나로 봅니다.</Strong></p></Copy><Copy><p>이 주방 역시 기존 천장 안의 조건을 고려하지 않고 계획할 수 있는 공간은 아니었습니다.</p><p>필요한 부분마다 서로 다른 높이를 반복하기보다, 주방 천장을 하나의 영역으로 정리하는 방식을 선택했습니다.</p><p>그 결과 기술적인 조건을 해결하는 동시에 천장의 변화가 주방과 다이닝 영역을 하나의 공간으로 읽히게 합니다.</p></Copy></div>
        <div className={`${evidenceCanvas} mt-10 grid gap-8 md:grid-cols-2 md:items-start`}>
          <Figure src="/projects/apartment-a/16-before-kitchen.webp" width={1400} height={1050} project="수원 살구골 현대7단지 99" status="BEFORE" caption="기존 주방의 천장과 공간 조건" alt="공사 전 수원 살구골 현대7단지 주방과 기존 천장" imageStageClassName="md:h-[420px]" imageClassName="md:h-full md:w-full md:object-cover md:object-[50%_44%]" />
          <Figure src="/projects/apartment-a/04-kitchen-overview.webp" width={1800} height={2400} project="수원 살구골 현대7단지 99" status="COMPLETED" caption="천장의 변화를 하나의 영역으로 정리한 주방과 다이닝" alt="천장 영역을 단정하게 정리한 수원 살구골 현대7단지 주방" imageStageClassName="md:h-[420px]" imageClassName="md:h-full md:w-full md:object-cover md:object-[50%_36%]" />
        </div>
      ...
      <Section><div><div className="max-w-[760px]"><Heading number="05" title="수납량을 늘려도 공간은 복잡해지지 않아야 합니다" /><Copy><p>필요한 물건과 기능이 많다고 해서 공간까지 복잡하게 보여야 하는 것은 아닙니다.</p><p>기능이 생길 때마다 작은 가구와 수납을 하나씩 더하면 공간은 쉽게 잘게 나뉩니다.</p><p>그래서 AND는 수납량뿐 아니라, <Strong>많은 기능이 몇 개의 큰 면과 구조 안에서 어떻게 하나의 질서로 읽히는지</Strong>를 봅니다.</p><p>이 주방에는 가족의 생활에 필요한 다양한 수납과 가전, 조리 기능이 필요했습니다.</p><p>각각을 독립적인 가구처럼 드러내기보다 큰 수납면과 아일랜드를 중심으로 정리했습니다.</p><p>멀리서 공간을 바라보면 먼저 읽히는 것은 여러 개의 작은 수납장이 아니라 <Strong>몇 개의 큰 면과 구조</Strong>입니다.</p></Copy></div>
        <div className="mt-14"><p className="mb-7 text-xs font-medium tracking-[0.24em] text-neutral-500">MORE FUNCTION, LESS VISUAL NOISE</p><div className={evidenceCanvas}><Figure src="/projects/apartment-b/19-kitchen-island.webp" width={1448} height={1086} project="화성 동탄역 모아미래도 84A" status="COMPLETED" caption="다양한 기능을 받아들이는 아일랜드를 외부에서는 하나의 단순한 구조로 정리했습니다." alt="아일랜드와 큰 수납면이 단순한 구조로 읽히는 동탄역 모아미래도 주방" className={evidenceSingle} /><Copy><p><Strong>필요한 기능이 많아질수록 더 많이 드러내는 것이 아니라, 더 큰 질서 안에서 정리하는 것이 중요합니다.</Strong></p></Copy><ProjectLink status="COMPLETED" href="/projects/apartment-b">동탄역 모아미래도에서 주방의 수납과 기능을 정리한 방식 보기 →</ProjectLink></div></div>
      </div></Section>
      ...
      <Section><div className="max-w-[760px]"><Heading number="06" title="가족의 생활은 서로를 방해하지 않아야 합니다" /><Copy><p>좋은 동선을 이야기할 때 가장 짧은 거리를 먼저 떠올리기 쉽습니다.</p><p>하지만 집에서는 항상 한 사람만 움직이지 않습니다.</p><p>누군가는 자고 있고, 누군가는 출근을 준비합니다.</p><p>한 사람은 일하고, 다른 사람은 쉬고 있을 수 있습니다.</p><p>그래서 주거공간의 동선은 몇 걸음을 줄이는 문제만으로 판단하기 어렵습니다.</p><p><Strong>좋은 동선은 서로 다른 생활이 같은 공간에서 불필요하게 충돌하지 않도록 만드는 일에 가깝습니다.</Strong></p></Copy></div>
        <div className="mt-14"><div className="max-w-[760px]"><Copy><p>이 집에서는 부부의 생활시간이 달랐습니다.</p><p>한 사람이 잠들어 있는 동안 다른 사람이 씻고, 옷을 고르고, 외출을 준비하는 일이 반복될 수 있었습니다.</p><p>기존에는 욕실과 드레스 기능을 사용하는 과정에서 침실을 반복해서 지나게 되는 흐름이었습니다.</p><p>AND는 공간 사이의 관계를 다시 정리하면서 <Strong>한 번 침실을 나온 뒤에는 준비를 마칠 때까지 침실을 반복해서 지나지 않아도 되는 흐름</Strong>을 만들었습니다.</p><p>침실은 이동을 위한 통로보다 휴식을 위한 공간으로 남기고, 욕실과 드레스 기능은 침실 밖 생활동선에서 이어질 수 있도록 관계를 정리했습니다.</p><p><Strong>핵심은 한 사람의 준비 과정이 다른 사람의 휴식 공간을 반복해서 가로지르지 않게 하는 것이었습니다.</Strong></p><p>동선의 가치는 이동거리 몇 미터보다 이런 생활의 차이에서 더 분명하게 나타납니다.</p></Copy></div>
          <p className="mb-7 mt-12 text-xs font-medium tracking-[0.24em] text-neutral-500">FLOW BETWEEN PEOPLE</p><div className={evidenceCanvas}><div className="grid gap-8 md:grid-cols-2 md:items-start"><Figure src="/projects/apartment-b/14-master-entry.webp" width={1086} height={1448} project="화성 동탄역 모아미래도 84A" status="COMPLETED" caption="침실을 나온 뒤 준비 공간으로 이어지는 안방 출입 영역" alt="침실 밖 준비 공간으로 이어지는 동탄역 모아미래도 안방 출입구" /><Figure src="/projects/apartment-b/15-dressing-room.webp" width={1086} height={1448} project="화성 동탄역 모아미래도 84A" status="COMPLETED" caption="침실을 반복해서 지나지 않고 사용할 수 있도록 관계를 정리한 드레스룸" alt="복도 생활동선과 연결되도록 확장한 동탄역 모아미래도 드레스룸" /></div>
          </div>
        </div>
        <div className="mt-20 border-t border-[#675B56]/25 pt-12 md:mt-28"><div className="max-w-[760px]"><h3 className="text-2xl font-light">일과 휴식도 같은 기준으로 봅니다</h3><Copy><p>같은 프로젝트에서 업무공간 역시 휴식공간 안에 단순히 기능을 더하는 방식으로 해결하지 않았습니다.</p><p>집에서 일하는 시간과 쉬는 시간이 서로 다른 영역에서 이루어질 수 있도록 독립적인 업무공간을 마련했습니다.</p><p>가족의 생활을 모두 가까이 모으는 것이 항상 좋은 것은 아닙니다.</p><p><Strong>때로는 적절히 분리하는 것이 서로의 생활을 더 편하게 만듭니다.</Strong></p></Copy></div><div className={`${evidenceCanvas} mt-12`}><Figure src="/projects/apartment-b/18-study-homebar.webp" width={1448} height={1086} project="화성 동탄역 모아미래도 84A" status="COMPLETED" caption="거실과 시선을 연결하면서도 업무와 휴식이 서로 다른 영역에서 이루어지는 독립 서재" alt="거실과 연결되면서 독립된 업무 영역을 이루는 동탄역 모아미래도 서재" className={evidenceSecondary} /><ProjectLink status="COMPLETED" href="/projects/apartment-b">동탄역 모아미래도에서 생활 동선을 재구성한 방식 보기 →</ProjectLink></div></div>
      </Section>
      ...
      <section className="mx-auto max-w-[960px] px-5 pb-32 text-center md:px-16 md:pb-48"><p className="text-[10px] font-medium tracking-[0.28em] text-neutral-500 md:text-xs">CONCLUSION</p><h2 className="mt-6 text-3xl font-light leading-[1.25] tracking-[-0.03em] break-keep md:text-[44px]">결국 수납이 아니라 생활의 질서를 설계합니다</h2><div className="mx-auto mt-10 max-w-[720px] space-y-5 text-[15px] leading-8 text-neutral-600 break-keep md:text-lg md:leading-[1.9]">
        <p>수납계획은 장의 개수를 정하는 일에서 시작하지 않습니다.</p><p>필요한 물건과 가족의 우선순위를 파악하고, 반복되는 행동에 맞춰 기능의 위치를 정합니다. 공간이 부족하다면 기능 사이의 관계와 위치를 다시 보고, 때로는 수납이 공간을 나누는 경계가 되기도 합니다.</p><p>그 모든 판단의 마지막에는 결국 사람이 있습니다.</p><p>한 사람의 움직임이 다른 사람의 휴식을 방해하지 않는지, 일과 휴식이 필요 이상으로 뒤섞이지 않는지, 매일의 생활이 자연스럽게 이어지는지를 봅니다.</p><p className="text-xl font-light text-[#4A433D]"><Strong>좋은 수납은 물건을 많이 감추는 기술이 아닙니다.</Strong></p><p>공간 안에서 물건과 행동, 그리고 사람의 관계가 자연스럽게 이어지도록 만드는 일입니다.</p><p className="text-xl leading-9 text-[#4A433D]"><Strong>우리가 설계하는 것은 수납장의 개수가 아니라, 그 안에서 이어질 생활의 질서입니다.</Strong></p>
      </div></section>
    </article><BackToTop />
  </main>;
}
