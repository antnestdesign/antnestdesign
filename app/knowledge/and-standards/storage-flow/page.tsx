...
export default function StorageFlowPage() {
  return <main className="min-h-screen bg-[#F3F0EB] text-[#4A433D]">
    ...
    <article>
      ...
      <Section>
        <div className="grid gap-12 lg:grid-cols-12 lg:gap-x-20 xl:gap-x-24">
          <div className="lg:col-span-5"><Heading number="01" title="수납보다 먼저 생활을 봅니다" /><Copy>
          ...
          <div className="lg:col-span-7 lg:pt-10">
            ...
            <div className="grid gap-9">
              <Figure src="/projects/cheongna-central-eileens-garden-84a/16-kitchen-overview-panorama.webp" width={4000} height={1179} project="청라 센트럴에일린의뜰 84A" status="DESIGN PROPOSAL" caption="아일랜드와 다이닝을 생활의 중심에 둔 주방 제안" alt="긴 아일랜드와 다이닝이 연결된 청라 센트럴에일린의뜰 주방 제안" priority />
              <Figure src="/projects/cheongna-central-eileens-garden-84a/21-utility-kitchen.webp" width={1391} height={1100} project="청라 센트럴에일린의뜰 84A" status="DESIGN PROPOSAL" caption="메인 주방과 분리된 조리 영역에 다른 비중을 둔 주방 제안" alt="메인 주방과 문으로 구분된 청라 센트럴에일린의뜰 보조주방 제안" className="ml-auto max-w-[560px]" />
            </div>
            <Copy><p className="text-xl font-light text-[#4A433D]">같은 평면이라고 같은 답이 나오는 것은 아닙니다.</p><p>하나는 아일랜드와 다이닝을 생활의 중심으로 보고 주방을 구성한 안이고, 다른 하나는 메인 주방과 별도의 조리 영역에 서로 다른 비중을 둔 안입니다.</p><p>두 안 중 하나가 항상 더 좋은 것은 아닙니다.</p><p><Strong>어떤 계획이 맞는지는 그 집에서 실제로 어떻게 요리하고, 먹고, 머무는지에 따라 달라집니다.</Strong></p></Copy>
      ...
      <Section><div className="grid gap-12 lg:grid-cols-12 lg:gap-x-20 xl:gap-x-24">
        <div className="lg:col-span-5"><Heading number="02" title="물건의 위치는 행동의 순서를 따라야 합니다" /><Copy>
        ...
        <div className="lg:col-span-7 lg:pt-10"><p className="mb-7 text-xs font-medium tracking-[0.24em] text-neutral-500">STORAGE FOLLOWS ACTION</p>
          <div className="grid gap-8 md:grid-cols-2 md:items-start"><Figure src="/projects/apartment-b/12-kitchen-front.webp" width={1448} height={1086} project="화성 동탄역 모아미래도 84A" status="COMPLETED" caption="냉장·보관 영역과 중앙 아일랜드가 실제 사용 흐름 안에서 이어집니다." alt="냉장 수납벽과 중앙 아일랜드가 함께 보이는 동탄역 모아미래도 주방" /><Figure src="/projects/apartment-b/19-kitchen-island.webp" width={1448} height={1086} project="화성 동탄역 모아미래도 84A" status="COMPLETED" caption="아일랜드에서 세척과 손질, 조리가 자연스럽게 이어지도록 기능의 관계를 정리했습니다." alt="세척과 조리 기능이 연결된 동탄역 모아미래도 주방 아일랜드" className="md:mt-20" /></div>
          <Copy><p>이 집은 요리를 자주 하는 가족의 생활을 기준으로 주방의 기능을 배치했습니다.</p><p>사진에서 독자가 봐야 하는 것은 가구의 형태가 아니라 <Strong>각 기능이 어떤 순서로 연결되는가</Strong>입니다.</p><p>이 원칙은 주방에만 적용되지 않습니다. 현관에는 귀가와 외출의 순서가 있고, 드레스룸에는 보관과 준비의 순서가 있습니다.</p><p><Strong>물건을 어디에 둘 것인가를 결정하기 전에, 그 물건을 언제 어떻게 사용하는지를 먼저 봐야 합니다.</Strong></p></Copy>
      ...
      <Section><div className="max-w-[760px]"><Heading number="03" title="공간이 부족하면 기능의 위치부터 다시 봅니다" /><Copy><p>공간이 부족할 때 가장 먼저 떠올리는 방법은 각 요소를 조금씩 줄이는 것입니다.</p><p>수납장을 줄이고, 식탁을 작게 바꾸고, 통로를 좁히면서 필요한 기능을 기존 위치에 모두 남겨두려 합니다.</p><p>하지만 하나의 공간 안에서 여러 기능이 서로 경쟁하고 있다면, 크기를 줄이는 것만으로는 근본적인 문제가 해결되지 않을 수 있습니다.</p><p><Strong>이럴 때 AND는 각각의 기능을 그대로 둔 채 얼마나 줄일 수 있는지를 보기보다, 기능 사이의 관계와 주변 동선까지 함께 놓고 위치를 다시 검토합니다.</Strong></p></Copy></div>
        <div className="mt-14"><p className="mb-7 text-xs font-medium tracking-[0.24em] text-neutral-500">RETHINKING POSITION</p><div className="grid gap-9 md:grid-cols-12 md:items-start">
          <Figure src="/projects/apartment-a/18-before-balcony.webp" width={1050} height={1400} project="수원 살구골 현대7단지 99" status="BEFORE" caption="활용도가 낮았던 기존 인접 영역" alt="기능을 옮기기 전 수원 살구골 현대7단지의 비어 있는 발코니" className="md:col-span-3" />
          <Figure src="/projects/apartment-a/07-utility-room.webp" width={1746} height={2400} project="수원 살구골 현대7단지 99" status="COMPLETED" caption="주방의 흐름을 유지하면서 인접 영역으로 기능의 위치를 조정했습니다." alt="냉장 기능과 유틸리티를 인접 영역으로 옮긴 수원 아파트 발코니" className="md:col-span-4 md:mt-20" />
          <Figure src="/projects/apartment-a/04-kitchen-overview.webp" width={1800} height={2400} project="수원 살구골 현대7단지 99" status="COMPLETED" caption="조리와 수납, 다이닝과 인접한 방으로 이어지는 흐름을 함께 회복한 본 주방" alt="조리 공간과 식탁, 방으로 향하는 통로가 함께 보이는 수원 아파트 주방" className="md:col-span-5" />
        </div><div className="mt-12 grid gap-8 lg:grid-cols-2 lg:gap-20"><Copy><p>기존 주방에서는 조리와 수납뿐 아니라 다이닝, 그리고 주방 옆 방으로 이어지는 이동까지 하나의 영역 안에서 함께 해결해야 했습니다.</p><p>가족이 식탁과 주방을 자연스럽게 오갈 수 있어야 했고, 동시에 주방 옆 방으로 향하는 통로가 가구나 식탁 때문에 방해받지 않아야 했습니다.</p><p>AND는 이 관계를 함께 살펴보면서 <Strong>주방의 기본적인 작업 흐름을 유지할 수 있는 범위 안에서 냉장 기능의 위치를 인접 영역으로 조정했습니다.</Strong></p></Copy><Copy><p>중요한 것은 냉장고를 옮겼다는 사실 자체가 아닙니다.</p><p><Strong>하나의 기능 위치를 바꾸면서 주방의 조리 흐름, 다이닝과의 연결, 그리고 인접한 방으로 이어지는 통로를 함께 정리할 수 있었다는 점입니다.</Strong></p><p>공간이 부족할 때 모든 것을 조금씩 줄이는 것이 항상 답은 아닙니다. <Strong>기능 하나의 위치를 다시 보는 선택이 서로 충돌하던 여러 공간의 관계를 함께 바꿀 수도 있습니다.</Strong></p></Copy></div>
      ...
      <aside className="mx-auto mb-32 max-w-[1060px] px-5 md:mb-48 md:px-16"><div className="border-y border-[#675B56]/30 py-12 md:py-16"><p className="text-[10px] font-medium tracking-[0.28em] text-neutral-500 md:text-xs">DESIGN NOTE</p><h2 className="mt-5 text-2xl font-light md:text-3xl">좋은 계획은 실제로 구현될 수 있어야 합니다</h2>
        ...
        <div className="mt-10 grid gap-8 md:grid-cols-2 md:items-start"><Figure src="/projects/apartment-a/16-before-kitchen.webp" width={1400} height={1050} project="수원 살구골 현대7단지 99" status="BEFORE" caption="기존 주방의 천장과 공간 조건" alt="공사 전 수원 살구골 현대7단지 주방과 기존 천장" /><Figure src="/projects/apartment-a/04-kitchen-overview.webp" width={1800} height={2400} project="수원 살구골 현대7단지 99" status="COMPLETED" caption="천장의 변화를 하나의 영역으로 정리한 주방과 다이닝" alt="천장 영역을 단정하게 정리한 수원 살구골 현대7단지 주방" className="md:max-w-[420px]" /></div>
        <p className="mt-10 text-xl font-light leading-8 text-[#675B56] break-keep">좋은 계획은 실제로 만들어졌을 때도 처음의 공간적 의도가 유지되어야 합니다.</p>
      </div></aside>

      <Section><div className="grid gap-12 lg:grid-cols-12 lg:gap-x-20 xl:gap-x-24"><div className="lg:col-span-5"><Heading number="04" title="수납은 때로 공간의 경계가 됩니다" /><Copy><p>수납은 보통 남는 벽을 따라 배치하는 가구로 생각하기 쉽습니다.</p><p>하지만 공간을 다시 구성할 때 수납은 물건을 담는 장소보다 더 큰 역할을 할 수 있습니다.</p><p><Strong>수납을 포함한 하나의 구조가 두 영역을 나누고, 시선을 정리하고, 각각의 공간에 서로 다른 성격을 만들 수도 있습니다.</Strong></p><p>침실에서 가장 먼저 보이는 면은 공간의 중심을 만듭니다.</p><p>이 프로젝트에서는 침대 헤드 영역에 새로운 구조를 두고, 그 뒤의 공간에 수납과 준비 기능을 연결했습니다.</p><p>침실에서 바라보면 하나의 정돈된 중심면으로 읽히지만, 반대편에서는 다른 생활기능을 받아들이는 영역이 됩니다.</p><p><Strong>하나의 구조가 침실의 배경인 동시에 두 영역을 구분하는 경계가 되는 것입니다.</Strong></p></Copy></div>
        <div className="lg:col-span-7 lg:pt-10"><p className="mb-7 text-xs font-medium tracking-[0.24em] text-neutral-500">STORAGE AS A BOUNDARY</p><div className="grid gap-8 md:grid-cols-2 md:items-start"><Figure src="/projects/cheongna-hanwha-kkumegreen-39a/07-master-bedroom-day.webp" width={1672} height={1125} project="청라 한화꿈에그린 100A" status="DESIGN PROPOSAL" caption="침실에서는 새로운 구조가 정돈된 중심면으로 읽힙니다." alt="침대 헤드 구조가 침실의 중심면을 만드는 청라 한화꿈에그린 제안" /><Figure src="/projects/cheongna-hanwha-kkumegreen-39a/09-master-bedroom-dressing-room-extension.webp" width={1708} height={1125} project="청라 한화꿈에그린 100A" status="DESIGN PROPOSAL" caption="반대편에서는 수납과 준비 기능을 연결하며 두 영역의 경계가 됩니다." alt="침대 헤드 구조 뒤에 수납과 준비 영역을 연결한 청라 한화꿈에그린 제안" className="md:mt-20" /></div>
          <Copy><p>수납은 반드시 기존 벽을 따라 추가되는 가구일 필요가 없습니다.</p><p><Strong>어디에 놓이고 어떤 관계를 만드느냐에 따라 수납을 포함한 구조 자체가 새로운 공간의 시작과 끝을 만들 수 있습니다.</Strong></p><p>수원 살구골 현대7단지에서도 하나의 구조가 현관에서는 수납의 경계가 되고, 거실에서는 중심면으로 작동했습니다.</p></Copy><ProjectLink status="DESIGN PROPOSAL" href="/projects/cheongna-hanwha-kkumegreen-39a">청라 한화꿈에그린에서 침실과 수납공간의 관계 보기 →</ProjectLink>
        </div></div>
      </Section>

      <Section><div className="grid gap-12 lg:grid-cols-12 lg:gap-x-20 xl:gap-x-24"><div className="lg:col-span-5 lg:col-start-8"><Heading number="05" title="수납량을 늘려도 공간은 복잡해지지 않아야 합니다" /><Copy><p>필요한 물건과 기능이 많다고 해서 공간까지 복잡하게 보여야 하는 것은 아닙니다.</p><p>필요한 기능이 생길 때마다 작은 가구와 수납을 하나씩 더하면 수납은 늘어나지만 공간에는 수많은 선과 덩어리가 남습니다.</p><p>그래서 AND는 수납의 개수만큼이나 <Strong>많은 기능이 공간 안에서 어떻게 하나의 질서로 읽히는지</Strong>를 봅니다.</p><p><Strong>필요한 수납과 기능이 많아질수록 그것들을 여러 작은 요소로 흩뜨리기보다, 몇 개의 큰 면과 구조 안에서 정리할 필요가 있습니다.</Strong></p><p>이 주방에는 가족의 생활에 필요한 다양한 수납과 가전, 조리 기능이 필요했습니다.</p><p>하지만 각각을 독립적인 가구처럼 드러내기보다 큰 수납면과 아일랜드를 중심으로 기능을 정리했습니다.</p><p>멀리서 공간을 바라보면 먼저 읽히는 것은 여러 개의 작은 수납장이 아니라 <Strong>몇 개의 큰 면과 구조</Strong>입니다.</p></Copy></div>
        <div className="lg:col-span-6 lg:col-start-1 lg:row-start-1 lg:pt-10"><p className="mb-7 text-xs font-medium tracking-[0.24em] text-neutral-500">MORE FUNCTION, LESS VISUAL NOISE</p><Figure src="/projects/apartment-b/12-kitchen-front.webp" width={1448} height={1086} project="화성 동탄역 모아미래도 84A" status="COMPLETED" caption="다양한 수납과 가전, 조리 기능을 큰 수납면과 아일랜드라는 단순한 구조 안에 정리했습니다." alt="큰 수납면과 아일랜드가 하나의 질서로 읽히는 동탄역 모아미래도 주방" /><Copy><p>아일랜드 역시 다양한 기능을 받아들이지만 외부에서는 하나의 단순한 형태로 인식됩니다.</p><p><Strong>필요한 기능이 많아질수록 더 많이 드러내는 것이 아니라, 더 큰 질서 안에서 정리하는 것이 중요합니다.</Strong></p><p>수납을 늘리는 것이 목적이 되면 공간은 쉽게 복잡해집니다. 하지만 생활에 필요한 기능을 먼저 파악하고 그것을 몇 개의 구조 안에 정리하면, <Strong>수납이 많아도 공간은 단순하게 읽힐 수 있습니다.</Strong></p></Copy><ProjectLink status="COMPLETED" href="/projects/apartment-b">동탄역 모아미래도에서 주방의 수납과 기능을 정리한 방식 보기 →</ProjectLink></div>
      </div></Section>
      ...
      <Section><div className="max-w-[760px]"><Heading number="06" title="가족의 생활은 서로를 방해하지 않아야 합니다" /><Copy><p>좋은 동선을 이야기할 때 가장 짧은 거리를 먼저 떠올리기 쉽습니다.</p><p>하지만 집에서는 항상 한 사람만 움직이지 않습니다.</p><p>누군가는 자고 있고, 누군가는 출근을 준비합니다.</p><p>한 사람은 일하고, 다른 사람은 쉬고 있을 수 있습니다.</p><p>그래서 주거공간의 동선은 몇 걸음을 줄이는 문제만으로 판단하기 어렵습니다.</p><p><Strong>좋은 동선은 서로 다른 생활이 같은 공간에서 불필요하게 충돌하지 않도록 만드는 일에 가깝습니다.</Strong></p></Copy></div>
        <div className="mt-14 grid gap-12 lg:grid-cols-12 lg:gap-x-20 xl:gap-x-24"><div className="lg:col-span-7"><p className="mb-7 text-xs font-medium tracking-[0.24em] text-neutral-500">FLOW BETWEEN PEOPLE</p><div className="grid gap-8 md:grid-cols-2 md:items-start"><Figure src="/projects/apartment-b/14-master-entry.webp" width={1086} height={1448} project="화성 동탄역 모아미래도 84A" status="COMPLETED" caption="침실을 나온 뒤 준비 공간으로 이어지는 안방 출입 영역" alt="침실 밖 준비 공간으로 이어지는 동탄역 모아미래도 안방 출입구" /><Figure src="/projects/apartment-b/15-dressing-room.webp" width={1086} height={1448} project="화성 동탄역 모아미래도 84A" status="COMPLETED" caption="침실을 반복해서 지나지 않고 사용할 수 있도록 관계를 정리한 드레스룸" alt="복도 생활동선과 연결되도록 확장한 동탄역 모아미래도 드레스룸" className="md:mt-20" /></div></div>
          <div className="lg:col-span-5 lg:pt-10"><Copy><p>이 집에서는 부부의 생활시간이 달랐습니다.</p><p>한 사람이 잠들어 있는 동안 다른 사람이 씻고, 옷을 고르고, 외출을 준비하는 일이 반복될 수 있었습니다.</p><p>기존 공간에서는 이런 준비 과정이 침실의 생활과 밀접하게 얽혀 있었습니다.</p><p>AND는 공간 사이의 관계를 다시 정리하면서 <Strong>한 번 침실을 나온 뒤에는 준비를 마칠 때까지 침실을 반복해서 지나지 않아도 되는 흐름</Strong>을 만들었습니다.</p><p>침실은 이동을 위한 통로보다 휴식을 위한 공간으로 남기고, 욕실과 드레스 기능은 침실 밖 생활동선에서 이어질 수 있도록 관계를 정리했습니다.</p><p><Strong>한 사람의 준비 과정이 다른 사람의 휴식 공간을 반복해서 가로지르지 않도록 했다는 것.</Strong></p><p>동선의 가치는 이동거리 몇 미터보다 이런 생활의 차이에서 더 분명하게 나타납니다.</p></Copy><ProjectLink status="COMPLETED" href="/projects/apartment-b">동탄역 모아미래도에서 침실과 생활 동선을 재구성한 방식 보기 →</ProjectLink></div>
        </div>
        <div className="mt-20 grid gap-10 border-t border-[#675B56]/25 pt-12 md:mt-28 lg:grid-cols-12 lg:gap-x-20"><div className="lg:col-span-5"><h3 className="text-2xl font-light">일과 휴식도 같은 기준으로 봅니다</h3><Copy><p>같은 프로젝트에서 업무공간 역시 휴식공간 안에 단순히 기능을 더하는 방식으로 해결하지 않았습니다.</p><p>집에서 일하는 시간과 쉬는 시간이 서로 다른 영역에서 이루어질 수 있도록 독립적인 업무공간을 마련했습니다.</p><p>가족의 생활을 모두 가까이 모으는 것이 항상 좋은 것은 아닙니다.</p><p><Strong>때로는 적절히 분리하는 것이 서로의 생활을 더 편하게 만듭니다.</Strong></p></Copy></div><Figure src="/projects/apartment-b/18-study-homebar.webp" width={1448} height={1086} project="화성 동탄역 모아미래도 84A" status="COMPLETED" caption="거실과 시선을 연결하면서도 업무와 휴식이 서로 다른 영역에서 이루어지는 독립 서재" alt="거실과 연결되면서 독립된 업무 영역을 이루는 동탄역 모아미래도 서재" className="lg:col-span-7" /></div>
      </Section>
